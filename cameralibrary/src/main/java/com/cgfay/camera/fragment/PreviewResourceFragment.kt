 ...
 */
class PreviewResourceFragment : Fragment() {

    private var onResourceChangeListener: OnResourceChangeListener? = null

    private val noneResource = ResourceData(
        "none",
    ...
    override fun onCreateView(
        ...
        return ComposeView(requireContext()).apply {
            setContent {
                PreviewResourceScreen(onSelect = {
                    onResourceChangeListener?.onResourceChange(it)
                })
            }
        }
    }

    fun addOnChangeResourceListener(listener: OnResourceChangeListener) {
        onResourceChangeListener = listener
    }

    interface OnResourceChangeListener {
        fun onResourceChange(data: ResourceData)
    }

    @Composable
    private fun PreviewResourceScreen(onSelect: (ResourceData) -> Unit) {
