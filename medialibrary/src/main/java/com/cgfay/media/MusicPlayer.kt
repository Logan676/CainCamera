...
class MusicPlayer : Closeable {
    ...
    companion object {
        ...
        init {
            NativeLibraryLoader.loadLibraries("ffmpeg", "musicplayer")
            native_init()
        }
        ...
        private fun postEventFromNative(ref: Any, what: Int, arg1: Int, arg2: Int, obj: Any?) {
            val mp = (ref as WeakReference<*>).get() as? MusicPlayer ?: return
    ...
    val duration: Float get() = _getDuration()
    val isLooping: Boolean get() = _isLooping()
    val isPlaying: Boolean get() = _isPlaying()

    private inner class EventHandler(mp: MusicPlayer, looper: Looper) : Handler(looper) {
        private val mMusicPlayer = mp
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                ...
                MEDIA_CURRENT -> mOnCurrentPositionListener?.onCurrentPosition(
                    mMusicPlayer,
                    msg.arg1.toFloat(),
                    msg.arg2.toFloat()
                )
